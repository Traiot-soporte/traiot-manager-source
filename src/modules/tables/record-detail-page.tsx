...
export function RecordDetailPage() {
  ...
  return (
    <div className="space-y-6">
      <Link className="inline-flex min-h-11 items-center gap-2 text-sm font-bold text-ink-800/55 hover:text-mint-600" to={basePath}><ArrowLeft className="size-4" />Volver a {table.name}</Link>
      <header className="flex flex-col gap-5 rounded-3xl bg-ink-950 p-6 text-white sm:flex-row sm:items-center sm:justify-between sm:p-8">
        <div><p className="text-xs font-black uppercase tracking-[0.18em] text-mint-400">Detalle</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">{getRowTitle(table, row.data)}</h1></div>
...
function RecordMessage({ backTo, title }: { readonly backTo: string; readonly title: string }) {
  return <section className="rounded-3xl bg-white p-8"><h1 className="text-xl font-black text-ink-950">{title}</h1><Link className="mt-5 inline-flex min-h-11 items-center text-sm font-bold text-mint-600" to={backTo}>Volver</Link></section>
}
